...
void main() {
  test('Context contains a Span', () {
    ...
    final testSpan = Span(
        ...
        sdk.Resource([]),
        sdk.InstrumentationLibrary('always_off_sampler_test', 'sampler_test_version'));
    final testContext = api.Context.current.withSpan(testSpan);

    final result =
        sdk.AlwaysOffSampler().shouldSample(testContext, traceId, testSpan.name, api.SpanKind.internal, [], []);

    expect(result.decision, equals(sdk.Decision.drop));
    expect(result.spanAttributes, equals(const <api.Attribute>[]));
    expect(result.traceState, same(traceState));
  });
  test('Context does not contain a Span', () {
    final traceId = api.TraceId([1, 2, 3]);
    final attributesList = [api.Attribute.fromBoolean('boolTest', true), api.Attribute.fromDouble('double', 0.3)];
    final testSpan = Span(
        'foo',
    ...
    final result = sdk.AlwaysOffSampler()
        .shouldSample(api.Context.root, traceId, testSpan.name, api.SpanKind.internal, attributesList, []);
