...
void main() {
  ...
  test('put valid values', () {
    ...
    expect(testTraceState.toString(), equals('key_0-1=value@2,key_0-2=value@1,key_@vendor=value@3'));
    expect(testTraceState.isEmpty, isFalse);
  ...
  test('create from string with invalid values', () {
    final testTraceState =
        TraceState.fromString('key_0-1=0,value2,key&0-2=value@1,key_@thisisalotlongerthan13characters=value@3');

    expect(testTraceState.get('key_0-1'), isNull); // Invalid value, comma.
    expect(testTraceState.get('key&0-2'), isNull); // Invalid key.
    expect(testTraceState.get('key_@thisisalotlongerthan13characters'), isNull); // Invalid vendor key.
    expect(testTraceState.toString(), equals(''));
    expect(testTraceState.isEmpty, isTrue);
    expect(testTraceState.size, equals(0));
  });
  ...
  test('put invalid values', () {
    final testTraceState = TraceState.empty()
      ...
      ..put('key_@thisisalotlongerthan13characters', 'value@3'); // Invalid vendor key.
  ...
  test('remove valid value', () {
    ...
    expect(testTraceState.toString(), equals('key_0-1=value@2,key_0-2=value@1'));
    expect(testTraceState.isEmpty, isFalse);
    expect(testTraceState.size, equals(2));
  });
  ...
  test('key regex, invalid vendor key', () {
    final matchResult = TraceState.validKeyRegex.matchAsPrefix('key_@thisisalotlongerthan13characters');
