...
@TestOn('vm')
import 'dart:async';
//import 'package:opentelemetry_sdk/sdk.dart' show trace;
import 'package:opentelemetry_sdk/src/sdk/open_telemetry.dart';
...
void main() {
  ...
  test('traceSync wants you to use trace instead', () {
    ...
    expect(() => traceSync('foo', () => Future<String?>.value()), throwsArgumentError);
  });
  ...
  test('trace throws future error', () async {
    ...
    await expectLater(trace('foo', () async => Future<String>.error(buzz)), throwsA(buzz));

    // Exception thrown from asynchronous code in async function.
    final bazz = Exception('bazz!!');
    await expectLater(trace('foo', () => Future<String>.error(bazz)), throwsA(bazz));
  });
}
